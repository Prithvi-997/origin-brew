...
export async function regeneratePages(
  ...
  try {
    ...
    const photoIdsToUse = pagesToRegenerate.flatMap(
      (page) => page.photoIds || []
    );
    const photosToUse = photos.filter((photo) =>
      photoIdsToUse.includes(photo.id)
    );
    ...
    const { data, error } = await supabase.functions.invoke("plan-photobook", {
      body: {
        ...
        keepLayouts: keepLayouts
          ? pagesToRegenerate.map((p) => p.layoutName)
          : undefined,
      },
    });
    ...
    const plan = data as {
      pages: Array<{
        ...
        frames: Array<{ frame_number: number; image_id: string }>;
      }>;
    };
